...
export default function Header() {
  ...
  return (
    <header className=" h-16 bg-white border-b border-gray-primary mb">
            ...
            {user ? (
             ...
             <div className="flex items-center cursor-pointer ">
              <Link to={`/p/${user.displayName}`}>
                <img src={`public/images/avatars/${user.displayName}.jpg`} alt={`${user.displayName} profile`} className=" rounded-full h-8 w-8 flex" />
              </Link>
             </div>
              </>
            ):(<>
